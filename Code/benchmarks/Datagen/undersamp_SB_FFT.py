...
for r in range(repetitions):
    for d, delta in enumerate(delta_values):
        ...
        t, Phi, w_true, e = generate_synthetic_data(N, D, rho, sigma,FFT=True)
        
        SBL_alg = SBL_Fast(t, Phi, max_iter=max_iter, threshold=threshold)
        w_cofem, _ = SBL_alg.fit()
...
data_file = os.path.join(data_dir, "undersampling_SB_FFT.npz")
...
plt.xlabel(r"$\delta$")
plt.ylabel(r"NRMSE [%]")
plt.title("Accuracy vs. Undersampling (SB FFT)")
plt.grid(True)
plt.tight_layout()
