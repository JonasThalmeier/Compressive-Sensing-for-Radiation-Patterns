...
for r in range(repetitions):
    for d, delta in enumerate(delta_values):
        ...
        t, Phi, w_true, e = generate_synthetic_data(N, D, rho, sigma,FFT=True, seed=42 + r)
...
data_file = os.path.join(data_dir, "undersampling_EM_FFT.npz")
...
errors = np.zeros((len(delta_values), repetitions))
for r in range(repetitions):
    for d, rho in enumerate(rho_values):
        N = int(np.floor(D*delta))
        t, Phi, w_true, e = generate_synthetic_data(N, D, rho, sigma,FFT=True, seed=42 + r)
        ...
        w_cofem, _ = SBL_alg.fit(np.array([10]))
        errors[d, r] = relative_error(w_cofem, w_true)
        print(f"Rho={rho}; Iteration {d+1}/{len(rho_values)}, Repetition {r+1}/{repetitions}, Error: {errors[d, r]:.4f}")
...
data_dir = os.path.join(base_dir, "data")
os.makedirs(data_dir, exist_ok=True)
data_file = os.path.join(data_dir, "sparsity_EM_FFT.npz")
np.savez(data_file, delta_values=delta_values, errors=errors)
data = np.load(data_file)
...
plt.plot(delta_values, mean_errors, label="EM (Gaussian)", linestyle=LINE_STYLES["EM"])


plt.xlabel(r"$\delta$")
plt.ylabel(r"NRMSE [%]")
plt.title("Accuracy vs. Undersampling (EM FFT)")
plt.grid(True)
plt.legend()
plt.tight_layout()
